...
async function generateWranglerConfig(
  ...
  const dbId = state.cloudflare?.d1DatabaseId || "<database-id>";
  const workerName = options.env 
  ...
  const wranglerConfig = {
    ...
    d1_databases: [
      {
        binding: "DB",
        database_name: state.cloudflare?.d1DatabaseName || config.name,
        database_id: dbId,
      },
    ],
    durable_objects: {
...
export async function deployCommand(options: DeployOptions): Promise<void> {
  ...
  const wranglerPath = await generateWranglerConfig(cwd, config, state, options);
