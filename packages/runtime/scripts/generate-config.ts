 ...
 *
 * Usage: node --import=tsx scripts/generate-config.ts
 *
...
import { DEFAULT_RUNTIME_NAMES } from "../src/runtime-names.js";
...
const staticSubstitutions = {
  WORKER_NAME: DEFAULT_RUNTIME_NAMES.workerName,
  WORKFLOW_NAME: DEFAULT_RUNTIME_NAMES.workflowName,
};
...
for (const [name, value] of Object.entries({ ...substitutions, ...staticSubstitutions })) {
...
for (const [name, value] of Object.entries(substitutions)) {
  const display = name.includes("TOKEN") || name.includes("SECRET") ? `${value.slice(0, 8)}...` : value;
