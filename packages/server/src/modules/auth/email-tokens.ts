...
export async function createEmailVerificationToken(
  ...
  try {
    ...
    const emailVerificationTokens = new EmailVerificationTokenRepository(env.DB);

    // Invalidate any existing tokens for this user
    await emailVerificationTokens.invalidateAllForUser(userId);

    await emailVerificationTokens.create(id, tokenHash, userId, expiresAt);
...
export async function verifyEmailToken(
  ...
  try {
    ...
    const row = await emailVerificationTokens.findByTokenHash(tokenHash);
    ...
    // Mark as consumed
    await emailVerificationTokens.consume(row.id);

    // Update user as verified
    await users.setEmailVerified(row.userId);
...
export async function createPasswordResetToken(
  ...
  try {
    ...
    const passwordResetTokens = new PasswordResetTokenRepository(env.DB);

    // Invalidate any existing tokens for this user
    await passwordResetTokens.invalidateAllForUser(userId);

    await passwordResetTokens.create(id, tokenHash, userId, expiresAt);
...
export async function verifyPasswordResetToken(
  ...
  try {
    ...
    const passwordResetTokens = new PasswordResetTokenRepository(env.DB);
    const row = await passwordResetTokens.findByTokenHash(tokenHash);
...
export async function consumePasswordResetToken(
  ...
  try {
    ...
    await passwordResetTokens.consume(row.id);
