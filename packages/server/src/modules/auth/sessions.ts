...
import { WebSessionRepository } from "../../data/index.js";
...
export async function createWebSession(
  ...
  try {
    ...
    const webSessions = new WebSessionRepository(env.DB);
    await webSessions.create({
      id: sessionId,
      userId,
      sessionTokenHash,
      csrfTokenHash,
...
export async function verifyWebSession(
  ...
  try {
    ...
    const webSessions = new WebSessionRepository(env.DB);
    const row = await webSessions.findByTokenHash(sessionTokenHash);
    ...
    if (Date.now() > row.expiresAt) {
      ...
      await webSessions.delete(row.id);
      return null;
    }

    // Update last_seen_at
    await webSessions.updateLastSeenAt(row.id);
...
export async function destroyWebSession(
  ...
  try {
    const webSessions = new WebSessionRepository(env.DB);
    await webSessions.delete(sessionId);
    return true;
  } catch (error) {
...
export async function destroyAllUserSessions(
  ...
  try {
    const webSessions = new WebSessionRepository(env.DB);
    await webSessions.deleteAllForUser(userId);
    return true;
  } catch (error) {
