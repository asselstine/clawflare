...
export async function invokeTool(args: {
  ...
  const tool = createTools(args.env, args.ctx, args.toolCtx).find(
...
function createStoreCodeTool(env: Env, _ctx?: ExecutionContext, toolCtx?: ToolContext): AgentTool {
  ...
  return {
    ...
    execute: async (
      ...
      const storedCode = new StoredCodeRepository(env.DB);
...
function createExecuteStoredCodeTool(env: Env, ctx?: ExecutionContext, toolCtx?: ToolContext): AgentTool {
  ...
  return {
    ...
    execute: async (
      ...
      const storedCode = new StoredCodeRepository(env.DB);
      const stored = await storedCode.get(workspaceId, p.name);
  ...
  return {
    ...
    execute: async (
      ...
      if (collection === "stored_code") {
        results = {
          storedCode: await storedCode.search(workspaceId, p.query ?? "*", limit),
          egressHandlers: [],
        };
      } else if (collection === "egress_handlers") {
        results = {
          storedCode: [],
          egressHandlers: await egressHandlers.search(workspaceId, p.query ?? "*", limit),
        };
      } else {
        const [storedCodeResults, egressHandlerResults] = await Promise.all([
          storedCode.search(workspaceId, p.query ?? "*", limit),
          egressHandlers.search(workspaceId, p.query ?? "*", limit),
        ]);
        results = {
