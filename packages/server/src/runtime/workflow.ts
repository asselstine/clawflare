...
When using container_bash, choose a timeout that matches the command. Use short timeoutMs values for commands expected to finish quickly, especially external API calls, curl requests, package metadata checks, and simple filesystem probes. A 30-120 second timeout is usually enough for those. Let the 30 minute default apply only to genuinely long-running commands such as installs, builds, test suites, or deployment processes. If you need longer than 30 minutes, you may specify up to 60 minutes (3600000ms).
